...
using AshenVoid.Functions.NPCChat;
using AshenVoid.Functions.NPCChat.Flows;

namespace AshenVoid.Content.女主
{
    // 目前是占位符，等女主贴图画好后再替换
    [AutoloadHead]
    public class ExamplePerson : ModNPC
    {
        ...
        public override string GetChat()
        {
            ...
            {
                chat.Add(Mod.GetLocalization($"{nameof(ExamplePerson)}.Dialogue.PartyGirlDialogue").Format(Main.npc[partyGirl].GivenName));
            }

            var text = Mod.GetLocalization($"{nameof(ExamplePerson)}.Dialogue.StandardDialogue1");
            // These are things that the NPC has a chance of telling you when you talk to it.
            chat.Add(Mod.GetLocalization($"{nameof(ExamplePerson)}.Dialogue.StandardDialogue1").Value);
            chat.Add(Mod.GetLocalization($"{nameof(ExamplePerson)}.Dialogue.StandardDialogue2").Value);
            chat.Add(Mod.GetLocalization($"{nameof(ExamplePerson)}.Dialogue.StandardDialogue3").Value);
            chat.Add(Mod.GetLocalization($"{nameof(ExamplePerson)}.Dialogue.StandardDialogue4").Value);
            chat.Add(Mod.GetLocalization($"{nameof(ExamplePerson)}.Dialogue.CommonDialogue").Value, 5.0);
            chat.Add(Mod.GetLocalization($"{nameof(ExamplePerson)}.Dialogue.RareDialogue").Value, 0.1);

            NumberOfTimesTalkedTo++;
            if (NumberOfTimesTalkedTo >= 10)
            {
                //This counter is linked to a single instance of the NPC, so if ExamplePerson is killed, the counter will reset.
                chat.Add(Mod.GetLocalization($"{nameof(ExamplePerson)}.Dialogue.TalkALot").Value);
            }
            ...
            if (chosenChat == Mod.GetLocalization($"{nameof(ExamplePerson)}.Dialogue.StandardDialogue4").Value)
            {
        ...
        {
            projType = ProjectileID.CrystalShard;//ModContent.ProjectileType<SparklingBall>();
            attackDelay = 1;
        }
